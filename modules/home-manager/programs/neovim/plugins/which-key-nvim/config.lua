...
local function register_directory_tree_mapping(key, rhs, what, opts) -- luacheck: ignore
	local spec = { name = "tree", [key] = { rhs, "Directory Tree " .. what } }
	log_mapping(spec)
	which_key.register({ ["<leader>t"] = spec }, opts)
end

local function register_find_mapping(key, rhs, what, opts) -- luacheck: ignore
...
local function register_refactor_mapping(key, rhs, what, opts) -- luacheck: ignore
