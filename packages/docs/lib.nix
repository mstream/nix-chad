...
let
  ...
  buildOptionsDocs = { modules, nixpkgsRef }:
    let
      inherit ((evalModules {
        inherit modules;
        check = false;
        ...
        let
          substituteDeclaration = decl:
            if hasPrefix chadPath (builtins.toString decl) then
              ...
              decl;
        in opt // {
          declarations = map substituteDeclaration opt.declarations;
        };
    };
  gitHubDeclaration = { ref, repo, subpath, user }: {
    url = "https://github.com/${user}/${repo}/blob/${ref}/${subpath}";
    name = "<${repo}/${subpath}>";
  };
in { inherit buildOptionsDocs; }
